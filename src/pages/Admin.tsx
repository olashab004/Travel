...
import { Plus, Edit2, Trash2, Save, X, Landmark, TreePine, Heart, Mountain, Compass, MapPin, Image as ImageIcon, Info, ShieldCheck, AlertCircle, Database, Lock, Key } from 'lucide-react';
...
export default function Admin() {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        ...
        if ((userDoc.exists() && userDoc.data().role === 'admin') || isDefaultAdmin) {
          ...
          // If no password is set, authorize automatically
  ...
  const handleVerifyPassword = (e: React.FormEvent) => {
    e.preventDefault();
    if (adminSettings?.adminPassword === passwordInput) {
      setIsAuthorized(true);
    } else {
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await firestoreService.updateAdminSettings({ adminPassword: newPassword });
      setAdminSettings({ ...adminSettings, adminPassword: newPassword });
  ...
  if (!isAuthorized) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
        <div className="max-w-md w-full bg-white rounded-3xl shadow-xl p-12 text-center space-y-8">
          <div className="inline-flex items-center justify-center w-20 h-20 rounded-full bg-indigo-100 text-indigo-600">
            <Lock size={40} />
          </div>
          <div className="space-y-2">
            <h1 className="text-3xl font-bold text-gray-900">Admin Password</h1>
            <p className="text-gray-500">Please enter the admin password to continue.</p>
          </div>
          <form onSubmit={handleVerifyPassword} className="space-y-4">
            <input 
              ...
              className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 transition-all text-center text-lg"
              value={passwordInput}
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex flex-wrap gap-4">
          <button 
            ...
            className="inline-flex items-center justify-center px-6 py-3 bg-white text-indigo-600 font-bold rounded-xl hover:bg-indigo-50 transition-all border border-indigo-100 shadow-sm"
          >
            <Key className="mr-2" size={20} />
            {adminSettings?.adminPassword ? 'Change Password' : 'Set Password'}
          </button>
          {!isEditing && (
            <button 
              onClick={handleSeed}
      ...
      {showPasswordChange && (
        <div className="bg-indigo-600 rounded-[2.5rem] shadow-2xl p-10 text-white relative overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full -mr-32 -mt-32 blur-3xl"></div>
          <div className="relative z-10">
            <div className="flex justify-between items-center mb-8">
              <h2 className="text-2xl font-bold flex items-center">
                <Key className="mr-3" size={24} />
                {adminSettings?.adminPassword ? 'Change Admin Password' : 'Set Admin Password'}
              </h2>
              <button 
            ...
            <form onSubmit={handleChangePassword} className="max-w-md space-y-6">
              <div>
                <label className="block text-sm font-bold mb-2 uppercase tracking-wider opacity-80">New Password</label>
                <input 
                  required
                  type="password" 
                  placeholder="Enter new password"
                  className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-xl focus:ring-2 focus:ring-white focus:bg-white/20 transition-all text-white placeholder-white/40"
                  value={newPassword}
              ...
              <div className="flex gap-4">
                <button 
                  type="submit"
                  className="px-8 py-3 bg-white text-indigo-600 font-bold rounded-xl hover:bg-indigo-50 transition-all shadow-xl"
                >
                  Save Password
                </button>
                <button 
